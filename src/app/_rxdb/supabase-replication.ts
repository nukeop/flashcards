...
> {
    ...
    public override async start(): Promise<void> {
        if (
            ...
            (this.options.pull.realtimePostgresChanges ||
                typeof this.options.pull.realtimePostgresChanges ===
                    'undefined')
        ) {
    ...
    > {
        ...
        if (lastCheckpoint && lastCheckpoint.modified) {
            ...
            const lastPrimaryKey = JSON.stringify(
                lastCheckpoint.primaryKeyValue,
            ); // TODO: Add test for a integer primary key
    ...
    private async handleInsertion(
        ...
        if (!error) {
            ...
            return [
                await this.fetchByPrimaryKey((doc as any)[this.primaryKey]),
            ];
        } else {
            throw error;
        }
    ...
    private async handleUpdate(
        ...
        const updateHandler =
            this.options.push?.updateHandler ||
            this.defaultUpdateHandler.bind(this);
    ...
    private async defaultUpdateHandler(
        ...
        Object.entries(row.assumedMasterState!).forEach(([field, value]) => {
            ...
            if (type === 'string' || type === 'number') {
                ...
                throw new Error(
                    `replicateSupabase: Unsupported field of type ${type}`,
                );
            }
        });
